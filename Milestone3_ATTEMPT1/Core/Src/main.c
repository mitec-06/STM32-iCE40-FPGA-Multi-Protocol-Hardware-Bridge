...
int main(void)
{
  ...
  while (1)
  {
   ...
   // while (LL_I2C_IsActiveFlag_BUSY(I2C1)){
      ...
      if (LL_I2C_IsActiveFlag_BUSY(I2C1)){
        msg = "Bus Stuck Error. (ERROR_BUS_STUCK FLAG)\n";
        UART_SendAndReadData(USART2, (const uint8_t *)msg, strlen(msg));
        LL_mDelay(50);
        continue;
      }
    ...
    msg = "Still workingx3..\n";
      UART_SendAndReadData(USART2, (const uint8_t *)msg, strlen(msg));
    if (I2C_Wait_With_Timeout(LL_I2C_SR1_SB, 14400) == ERROR_TIMEOUT){
      msg = "Timeout Error. (ERROR_TIMEOUT FLAG)\n";
      UART_SendAndReadData(USART2, (const uint8_t *)msg, strlen(msg));
  ...
  //  while (!LL_I2C_IsActiveFlag_SB(I2C1)){}
   ...
   // while (!LL_I2C_IsActiveFlag_ADDR(I2C1) && !LL_I2C_IsActiveFlag_AF(I2C1)){}
    ...
    if (I2C_Wait_With_Timeout(LL_I2C_SR1_ADDR, 14400) == ERROR_NACK){
      msg = "NACK Error. (ERROR_NACK FLAG)\n";
      UART_SendAndReadData(USART2, (const uint8_t *)msg, strlen(msg));
      LL_I2C_GenerateStopCondition(I2C1);
      LL_mDelay(50);
      continue;
  ...
  // failed condition, no address detected  
     ...
     /* if (I2C_Wait_With_Timeout(LL_I2C_SR1_TXE, 14400) != I2C_SUCCESS){
        msg = "First Byte Transmission Failed. \n";
        UART_SendAndReadData(USART2, (const uint8_t *)msg, strlen(msg));
        LL_I2C_GenerateStopCondition(I2C1);
        LL_mDelay(50);
        continue;
   */   }
      ...
      if (I2C_Wait_With_Timeout(LL_I2C_SR1_TXE, 14400) != I2C_SUCCESS){
        msg = "Second Byte Transmission Failed. \n";
        UART_SendAndReadData(USART2, (const uint8_t *)msg, strlen(msg));
        LL_I2C_GenerateStopCondition(I2C1);
        LL_mDelay(50);
      }
